    ...
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="font-semibold text-xl leading-tight" style="color:#1F4E79;">
                Editar Gestión — {{ $gestion->anio }} {{ ucfirst($gestion->periodo) }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route('admin.gestiones.update', $gestion->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="anio" name="anio" type="number" class="mt-1 block w-full"
                                :value="old('anio', $gestion->anio)" min="2020" max="2099" required />
                        ...
                        <div>
                            ...
                            <select id="periodo" name="periodo"
                                ...
                                <option value="primero" {{ old('periodo', $gestion->periodo)==='primero'?'selected':'' }}>Primero</option>
                                <option value="segundo" {{ old('periodo', $gestion->periodo)==='segundo'?'selected':'' }}>Segundo</option>
                            </select>
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-4">
                        <div>
                            ...
                            <x-text-input id="fecha_inicio" name="fecha_inicio" type="date" class="mt-1 block w-full"
                                :value="old('fecha_inicio', $gestion->fecha_inicio)" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="fecha_fin" name="fecha_fin" type="date" class="mt-1 block w-full"
                                :value="old('fecha_fin', $gestion->fecha_fin)" required />
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-4">
                        <div>
                            ...
                            <x-text-input id="cupo_por_carrera" name="cupo_por_carrera" type="number"
                                class="mt-1 block w-full" :value="old('cupo_por_carrera', $gestion->cupo_por_carrera)" min="1" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="monto_inscripcion" name="monto_inscripcion" type="number"
                                class="mt-1 block w-full" :value="old('monto_inscripcion', $gestion->monto_inscripcion)"
                    ...
                    @if($gestion->activa)
                    <div class="mt-4 p-3 rounded-md text-sm bg-yellow-50 border border-yellow-200 text-yellow-800">
                        Esta es la gestión activa. Los cambios afectan al sistema en curso.
                    </div>
                    @endif
